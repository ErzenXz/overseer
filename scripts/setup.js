#!/usr/bin/env node
/**
 * MyBot Interactive Setup Wizard
 * 
 * This script guides you through the initial setup of MyBot:
...
async function setup() {
  console.clear();
  log.header('╔════════════════════════════════════════╗');
  log.header('║   MyBot Interactive Setup Wizard      ║');
  log.header('╚════════════════════════════════════════╝');
  
  log.info('This wizard will help you set up MyBot with all required configurations.\n');
  ...
  config.DATABASE_PATH = await question('Database path', './data/mybot.db');
    ...
    while (!tokenValid) {
      ...
      if (tokenValid) {
        ...
        if (useWebhook.toLowerCase() === 'y') {
          config.TELEGRAM_WEBHOOK_DOMAIN = await question('Webhook domain (e.g., https://mybot.example.com)');
