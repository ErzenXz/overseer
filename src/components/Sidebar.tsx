...
import { useTweakcnTheme } from "@/components/TweakcnThemeProvider";
...
export function Sidebar({ user, permissions }: SidebarProps) {
 ...
 const { themes, currentTheme, setCurrentTheme, isLoading } = useTweakcnTheme();
        ...
        return (
         <div key={item.name}>
          <Link
           href={item.href}
           className={cn(
            "flex items-center gap-3 px-3 py-2 rounded-md text-[13px] font-medium transition-colors duration-200",
            isActive
   ...
   <div className="p-4 border-t border-border/50 bg-muted/30 m-3 rounded-xl">
    ...
    <div className="mb-3 space-y-1.5">
     ...
     <NativeSelect
      ...
      aria-label="Select tweakcn theme"
     >
      <option value="default">Default (shadcn)</option>
      {themes.map((theme) => (
