# 🚀 MyBot Deployment - Quick Reference Card
...
docker-compose restart mybot-web   # Restart specific service
```

### Systemd
```bash
sudo systemctl start mybot.service              # Start web server
sudo systemctl status mybot.service             # Check status
sudo journalctl -u mybot.service -f             # View logs
sudo systemctl restart mybot-telegram.service   # Restart Telegram bot
```
...
sudo tail -f /var/log/nginx/mybot*.log     # Nginx logs
pm2 logs mybot-web                         # PM2 logs
```
...
nginx/mybot.conf          # Nginx configuration
