/**
 * Security Audit Skill Implementation
...
// Secret patterns with higher accuracy
...
export async function scanDependencies(params: {
  ...
  try {
    ...
    // Known vulnerable packages (simplified - in real implementation, use a vulnerability database)
    const knownVulnerable: Record<string, { severity: SecurityVulnerability["severity"]; description: string; fixed?: string }> = {
    ...
    // Check for lock file
    const hasLockFile = existsSync(join(project_path, "package-lock.json")) ||
                        existsSync(join(project_path, "yarn.lock")) ||
...
export async function checkSecrets(params: {
  ...
  const secrets: Array<{ type: string; file: string; line: number; masked: string }> = [];
  let filesScanned = 0;
...
export const checkSecretsSchema = z.object({
