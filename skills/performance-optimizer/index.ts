/**
 * Performance Optimizer Skill Implementation
...
export interface BundleAnalysis {
  ...
  dependencies: {
    total: number;
    large: Array<{ name: string; reason: string }>;
    duplicates: string[];
  };
  issues: PerformanceIssue[];
  recommendations: string[];
  estimated_bundle_size?: string;
}
...
// Large/heavy dependencies to flag
 ...
 */
export async function analyzeBundle(params: {
  project_path: string;
}): Promise<BundleAnalysis> {
  const { project_path } = params;
  const issues: PerformanceIssue[] = [];
  const recommendations: string[] = [];
  const largeDeps: Array<{ name: string; reason: string }> = [];
  ...
  try {
    ...
    const hasNextConfig = existsSync(join(project_path, "next.config.js")) || 
                          existsSync(join(project_path, "next.config.mjs"));
    const hasViteConfig = existsSync(join(project_path, "vite.config.js")) ||
                          existsSync(join(project_path, "vite.config.ts"));
 ...
 */
export async function checkCodePerformance(params: {
  file_path: string;
}): Promise<{
  analyzed: boolean;
  file: string;
  ...
  try {
    ...
    const optimization_potential = 
      summary.critical > 0 || summary.high > 2 ? "high" :
      summary.high > 0 || summary.medium > 3 ? "medium" : "low";
 ...
 */
export async function analyzeDatabaseQueries(params: {
  queries: string[];
}): Promise<{
  analyzed: number;
  issues: PerformanceIssue[];
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    // Check for N+1 pattern indicator
 ...
 */
export async function suggestCaching(params: {
  description: string;
  ...
  strategies: Array<{
    name: string;
    type: "memory" | "redis" | "cdn" | "browser" | "http";
  ...
  // Determine caching strategies based on description
  ...
  if (requests_per_minute && requests_per_minute > 1000) {
    strategies.push({
      name: "Edge Caching",
      type: "cdn",
...
export const checkCodePerformanceSchema = z.object({
  file_path: z.string().describe("Path to file to analyze"),
});

export const analyzeDatabaseQueriesSchema = z.object({
  queries: z.array(z.string()).describe("SQL queries to analyze"),
});

export const suggestCachingSchema = z.object({
