/**
 * Deploy Assistant Skill Implementation
 * Help with deployments, environment setup, and releases
 */
 ...
 */
export async function checkDeployReadiness(params: {
  project_path: string;
  environment?: string;
  ...
  // Check for Dockerfile (if Docker deployment)
  ...
  const score = Math.round(
    (passedRequired / Math.max(totalRequired, 1)) * 70 +
    (passedOptional / Math.max(totalOptional, 1)) * 30
  );
 ...
 */
export async function generateDeployChecklist(params: {
  project_type: string;
  environment?: string;
 ...
 */
export async function verifyEnvironment(params: {
  project_path: string;
  env_file?: string;
  ...
  const variables: EnvironmentVerification["variables"] = [];
  ...
  // Read .env.example to know expected variables
  const expectedVars: Set<string> = new Set();
  ...
  const setVars: Set<string> = new Set();
...
export const generateDeployChecklistSchema = z.object({
  project_type: z.string().describe("Type of project"),
  environment: z.string().optional().describe("Target environment"),
});

export const verifyEnvironmentSchema = z.object({
  project_path: z.string().describe("Path to the project"),
